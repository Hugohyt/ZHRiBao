...
extern BOOL isLoadingWebView2;

@implementation CollectionWebview

- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    self.wkWebView = [[WKWebView alloc] init];
    self.scrollView = [[UIScrollView alloc] init];
    self.scrollView.frame = CGRectMake(0, 80, [UIScreen mainScreen].bounds.size.width, 852);
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(layoutNewWebView) name:@"newPageForCollection" object:nil];
    return self;
}
...
- (void)loadView {
    ...
    NSLog(@"url:%@",self.allArray[self.page - 1][@"URL"]);
    NSURL *urlWeb = [NSURL URLWithString:self.allArray[self.page - 1][@"URL"]];
    NSURLRequest *webRequest = [[NSURLRequest alloc] initWithURL:urlWeb];
    [self.wkWebView loadRequest:webRequest];
...
- (void)dealloc {
    // 移除观察者
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"newPageForCollection" object:nil];
}
